...
export default function LoginScreen({ navigation }: RootStackScreenProps<"Login">) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Cognito pedindo nova senha → aparece input
      ...
      if (error?.name === "UserNotConfirmedException") {
        ...
        showModal("error", "Erro", error?.message || "Algo deu errado. Tente novamente.");
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleConfirmNewPassword = async () => {
    if (!newPassword.trim()) return Alert.alert("Erro", "Digite uma nova senha.");
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        >
          <Text style={styles.buttonText}>
            {loading ? "Aguarde..." : requireNewPassword ? "Salvar Nova Senha" : "Logar"}
          </Text>
        </TouchableOpacity>
      </ScrollView>
    </KeyboardAvoidingView>
...
const styles = StyleSheet.create({
  ...
  scrollContainer: { flexGrow: 1, justifyContent: "space-around", alignItems: "center", padding: 20 },
  ...
  linksContainer: { width: "100%", flexDirection: "row", justifyContent: "space-between", paddingHorizontal: 5 },
  ...
  modalButton: { backgroundColor: "#191970", padding: 10, borderRadius: 5, alignItems: "center", width: "100%" },
