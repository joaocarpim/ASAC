...
export async function ensureUserExistsInDB(userId: string, username?: string, email?: string) {
  ...
  const res: any = await client.graphql({ query: GET_USER, variables: { id: userId } });
  ...
  const createRes: any = await client.graphql({ query: CREATE_USER, variables: { input } });
...
export async function getModuleProgressByUser(userId: string, moduleId: string) {
  ...
  `;
  const res: any = await client.graphql({ query: QUERY, variables: { userId, moduleId } });
...
export async function createModuleProgress(userId: string, moduleId: string, moduleNumber: number) {
  ...
  const res: any = await client.graphql({ query: MUT, variables: { input } });
...
export async function registerCorrect(userId: string, progressId: string) {
  ...
  `;
  const getRes: any = await client.graphql({ query: GET, variables: { id: progressId } });
 ...
 */
export async function canStartModule(userId: string, moduleNumber: number, moduleIdPrev?: string) {
  if (moduleNumber <= 1) return true;
  // find previous module progress: we don't have moduleIdPrev in all cases
  // We'll query listModuleProgresses filtering by userId and moduleNumber = moduleNumber-1
  const client = generateClient();
  const LIST = /* GraphQL */ `
  ...
  const res: any = await client.graphql({ query: LIST, variables: { filter } });
